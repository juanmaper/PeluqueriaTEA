...
class JuegoAsociacionSonidosViewModel() : ViewModel() {
    ...
    fun inicializarReproductor(context: Context) {
       if (listaObjetos.size == 0) {
           listaObjetos.add(ObjetoSonidoPeluqueria(context, "Secador", R.raw.audio_secador))
           listaObjetos.add(ObjetoSonidoPeluqueria(context, "Maquinilla", R.raw.audio_maquinilla))
           listaObjetos.add(ObjetoSonidoPeluqueria(context, "Spray", R.raw.audio_spray))
           listaObjetos.add(ObjetoSonidoPeluqueria(context, "Tijeras", R.raw.audio_tijeras))

           listaObjetos.shuffle()
           Log.d(TAG, "Orden de la lista: ${listaObjetos[0].nombreObjeto}, " +
